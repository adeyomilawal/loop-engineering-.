...
export function computeScore(signals) {
    ...
    let level = 'L0';
    if (score >= 75 && signals.verifier.present && signals.stateFile.present)
        level = 'L3';
    else if (score >= 55 && signals.triage.present)
        level = 'L2';
    else if (score >= 35 && signals.stateFile.present)
        level = 'L1';
    else
        level = 'L0';
    const assessment = score >= 80 ? 'Strong loop readiness — unattended possible with human gates.' :
